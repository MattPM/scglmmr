...
Below shows 2 steps. 1: Specify a varying intercept model using [lme4 symbolic model formula](https://arxiv.org/pdf/1911.08628.pdfs). The `1|subjectID` term treats individual as a random effect. This partially pools the estimate for baseline expression toward the global trend for each cell type by fitting a normal distribution to baseline expression (for each gene in the genome separately). 
